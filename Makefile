BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = Das_Festland_am_Suedpol
TITLE_NAME = "Das Festland am Südpol.Die Expedition zum Südpolarland in den Jahren 1898-1900."
...
$(BUILD)/epub/$(OUTPUT_FILENAME).epub: $(MAKEFILE) $(METADATA) $(CHAPTERS) $(CSS_FILE) $(CSS_FILE_KINDLE) $(IMAGES) \
	...
	mkdir -p $(BUILD)/epub
	$(PANDOC) $(ARGS) --from markdown+grid_tables+raw_html+fenced_divs+fenced_code_attributes+bracketed_spans --to epub+raw_html --resource-path=$(IMAGES_FOLDER) --epub-cover-image=$(COVER_IMAGE) -o $@  $(PREFACE_EPUB) $(CHAPTERS)
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA_PDF) $(CHAPTERS_HTML_PDF) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Festland_am_Suedpol_*.jpg .
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Festland_am_Suedpol_*.jpg  cover.jpg logo.jpg
